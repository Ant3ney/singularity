...
let switchMeta /* This object must follow a strict structure */ = {
	...
	formatLandingScreen: rawS => {
		let title = formatBoldsBreaksAndSpans(rawS?.title) || null;
		let subtitle = formatBoldsBreaksAndSpans(rawS?.subtitle) || null;
	...
	formatProduct: rawP => {
		let formatedSection = {
			type: rawP._type,
			landingScreen: switchMeta.formatLandingScreen(rawP.landingProductBanner),
			pricingAndContent: formatPricingAndContent(rawP.pricingAndContent),
			tecnicalDebtPolicy: switchMeta.formatProductsBanner(rawP.tecnicalDebtPolicy),
	...
	formatProductsDisplay: async rawS => {
		...
		let rawProducts: any = await getProductsData();
...
function formatComponentList(rawS) {
	...
	for (let i = 0; i < rawS.components.length; i++) {
		...
		newItem = {
			...
			description: currentComponent.description ? currentComponent.description : null,
			icon: currentComponent.icon ? getImageFromRaw(currentComponent, 'icon') : null,
...
function formatPriceTabel(rawS) {
	if (!rawS || !rawS.choices) {
		console.error('Given Raw section or rawS.choices camback as null or undefined');
		return {};
	}
	...
	for (let i = 0; i < rawS.choices.length; i++) {
		...
		newChoice = {
			...
			button: currentChoice.href
				? {
						title: currentChoice.actionTitle ? currentChoice.actionTitle : 'Start',
...
function formatProductsBannerSlide(rawS) {
	let formatedTitle = rawS.title ? formatBoldsBreaksAndSpans(rawS.title) : null;

	let displayImage = rawS.displayImage ? getImgUrlFromFileName(rawS.displayImage.asset._ref) : null;
	return {
		title: formatedTitle,
