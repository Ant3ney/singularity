...
export default function ContactUs({ mt }: ContactUsProps) {
  let [name, setName] = useState<string | null>(null);
  let [email, setEmail] = useState<string | null>(null);
  let [initalMessage, setInitalMessage] = useState<string | undefined>(undefined);
  ...
  async function submitedForm(e: FormEvent<HTMLFormElement>) {
    let messageTextarea = document.querySelector<HTMLTextAreaElement>("#message-form-textarea");
    ...
    if (!isEmailValid(email)) {
	  ...
	  if(!response.ok || res.fail) {
		  alert("Faild to record response. Please contact us directly at anthonycavuoti@gmail.com");
		  return;
	  } else {
		  window.gtag_report_conversion(
			  "https://thankyou.singularityplanet.com/"
		  );
		  /* setTimeout(() => {

			  window.top.location.href = "https://thankyou.singularityplanet.com/";
